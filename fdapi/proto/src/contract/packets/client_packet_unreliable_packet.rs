...
pub struct ClientPacketUnreliablePacket {
  packet_type: String,
  data: String,
}

impl Packet for ClientPacketUnreliablePacket {}

impl TryFrom<&Vec<u8>> for ClientPacketUnreliablePacket {
  type Error = Error;
  fn try_from(byte_vector: &Vec<u8>) -> Result<Self, Self::Error> {
    ...
    Ok(ClientPacketUnreliablePacket { packet_type, data })
  }
}

impl Into<Vec<u8>> for ClientPacketUnreliablePacket {
  fn into(self) -> Vec<u8> {
